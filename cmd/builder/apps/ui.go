...
type model struct {
	...
	pickerIndex map[string]int // tracks picker selection by field key
	focus       int
	width       int
	height      int
	err         error
...
func initialModel() model {
	fields := []ui.Field{
		{Label: "Name", Key: "name", Placeholder: "bacnet-server", Default: ""},
		{Label: "Version", Key: "version", Placeholder: "1.0.0", Default: "1.0.0"},
		{Label: "Arch", Key: "arch", Choices: []string{"amd64", "amd64-win", "arm64", "armv7"}},
		{Label: "Binary", Key: "binary", Placeholder: "./path/to/binary", Default: ""},
		{Label: "Extra Files", Key: "files", Placeholder: "config.json,data.yaml", Default: ""},
		{Label: "Port", Key: "port", Placeholder: "4000", Default: ""},
		{Label: "Health URL", Key: "health", Placeholder: "http://localhost:4000/healthz", Default: ""},
		{Label: "Args", Key: "args", Placeholder: "--flag1,--flag2", Default: ""},
...
func (m model) updateConfirm(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if ui.KeyMatches(msg, "g", "enter") {
		...
		return m, func() tea.Msg {
			zipPath, err := appbuilder.PackageApp(opts)
...
func (m model) View() string {
	header := ui.TitleStyle.Render("App Packager")
	sub := ui.MutedStyle.Render("Package a bios app into an installable .zip")
...
func (m model) renderForm() string {
	var lines []string
	lines = append(lines, ui.LabelStyle.Render("Step 1/2 · Configure your app package"))
	lines = append(lines, "")
...
func (m model) renderBuilding() string {
	...
	return strings.Join([]string{
		ui.LabelStyle.Render("Packaging app..."),
		"",
...
func (m model) renderDone() string {
	return strings.Join([]string{
		ui.OkStyle.Render("✔ App packaged successfully"),
		"",
...
func (m model) toOptions() (appbuilder.AppOptions, error) {
	...
	if vals["name"] == "" {
		return appbuilder.AppOptions{}, fmt.Errorf("name is required")
	}
	if vals["binary"] == "" {
		return appbuilder.AppOptions{}, fmt.Errorf("binary path is required")
	}

	opts := appbuilder.AppOptions{
		Name:      vals["name"],
		Version:   ui.Fallback(vals["version"], "0.1.0"),
		Arch:      vals["arch"],
		Binary:    vals["binary"],
		HealthURL: vals["health"],
		OutputDir: ui.Fallback(vals["output"], "./dist"),
	}

	if vals["port"] != "" {
		var p int
		fmt.Sscanf(vals["port"], "%d", &p)
		opts.Port = p
	}
...
// RunInteractive launches the TUI for app packaging.
func RunInteractive() {
