...
import { ProjectFormData, ProjectFormErrors, PROJECT_STATUSES, PROJECT_TYPES } from '@features/project/types/project.types';
...
export function ProjectFormFields({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <Label>
          Project Code <span className="text-[var(--rubix-destructive)]">*</span>
        </Label>
        <Input
          type="text"
          value={formData.projectCode}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange('projectCode', e.target.value)}
          className={`${formErrors.projectCode ? 'border-[var(--rubix-destructive)]' : ''} ${isEditing ? 'bg-[var(--rubix-muted)]' : ''}`}
          disabled={disabled || isEditing}
          placeholder={isEditing ? undefined : 'e.g., WP-001'}
          title={isEditing ? 'Project code cannot be changed' : undefined}
        />
        {formErrors.projectCode && (
          <div className="text-xs text-[var(--rubix-destructive)] mt-1">{formErrors.projectCode}</div>
        )}
        {isEditing && (
      ...
      <div>
        <Label>
          Project Type <span className="text-[var(--rubix-destructive)]">*</span>
        </Label>
        <select
          value={formData.projectType}
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onChange('projectType', e.target.value)}
