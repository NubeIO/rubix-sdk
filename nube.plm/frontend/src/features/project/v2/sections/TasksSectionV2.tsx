...
export function TasksSectionV2({ project, client, onStatsUpdate }: TasksSectionV2Props) {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const nodes = await client.queryNodes({
        filter: `type is "plm.task" and parent.id is "${project.id}"`,
      });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Track and manage project development tasks
            </p>
          </div>
        </div>
