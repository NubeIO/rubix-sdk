...
export function BOMSectionV2({ project, client, onStatsUpdate }: BOMSectionV2Props) {
  ...
  const fetchBOMItems = async () => {
    try {
      setIsLoading(true);
      console.log('[BOMSectionV2] Fetching BOM items for project:', project.id);

      // Query for BOM items
      const result = await client.queryNodes({
        filter: `parent.id is "${project.id}" and type is "plm.bom"`,
      });
  ...
  const handleAddBOMItem = async (bomItem: {
    ...
    await client.createNode(project.id, {
      type: 'core.document',
      profile: 'plm-bom',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage components and materials for this project
          </p>
        </div>
