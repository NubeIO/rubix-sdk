...
import type { SectionId } from '../ProjectDetailPageV2';
...
interface SidebarNavigationProps {
  ...
  projectType?: 'software' | 'hardware' | 'project';
  stats: {
    totalTasks: number;
    totalTickets?: number;
...
export function SidebarNavigation({
  ...
  // Filter out hardware-only sections for software/project types
  const navItems = allNavItems.filter(item => {
    if (projectType === 'project') {
      // Projects only get: overview, basic-info, tasks, tickets, system-info
      if (item.id === 'bom' || item.id === 'manufacturing' || item.id === 'pricing') {
        return false;
      }
    }
    if (projectType === 'software') {
      if (item.id === 'bom' || item.id === 'manufacturing') {
        return false;
      }
    }
    return true;
