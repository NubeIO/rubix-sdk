...
export function TasksDataTable({ tasks, projects, client, onEdit, onDelete, view, onViewChange }: TasksDataTableProps) {
  console.log('[TasksDataTable] Rendering with:', { tasksCount: tasks.length, projectsCount: projects.length });
  ...
  // Create project lookup map (memoized)
  ...
  const columns: ColumnDef<Task>[] = [
    ...
    {
      id: 'project',
      accessorFn: (row) => projectMap[row.parentId || '']?.name || 'Unknown',
      header: ({ column }) => {
        return (
          <Button
            variant="ghost"
      ...
      cell: ({ row }) => {
        const task = row.original;
        const project = projectMap[task.parentId || ''];
        return (
